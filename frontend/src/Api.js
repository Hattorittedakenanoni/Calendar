...
// Api.js の一番下などに追記してください

// 面接候補日の一覧取得
export async function getProposals() {
  const response = await fetch(`${API_BASE}/proposals`);
  if (!response.ok) throw new Error('候補日の取得に失敗しました');
  return response.json();
}
...
export async function createProposal(dateText) {
  ...
  if (!response.ok) throw new Error('候補日の保存に失敗しました');
  return response.json();
}
...
export async function deleteProposal(propId) {
  ...
  if (!response.ok) throw new Error('候補日の削除に失敗しました');
  return response.json();
}